...
namespace AMLCore.Injection.Game.Resource
{
    ...
    class Package : IDisposable
    {
        ...
        private static byte[] _Seed = new byte[] { 0x89, 0x49 };
            ...
            using (MemoryStream ms = new MemoryStream(buffer))
            {
                using (BinaryReader br = new BinaryReader(ms))
                {
                    for (int i = 0; i < a; ++i)
                    {
                        ...
                        if (!str.Contains('?'))
                        {
                            ret.FileList[str] = new StreamDecryptProvider(file, offset, length);
                        }
                    }
                }
            }
        ...
        private static List<string> ListFile(Package p, string prefix, string postfix)
        {
            return p.FileList.Keys.Where(k => k.StartsWith(prefix) && k.EndsWith(postfix)).ToList();
        }

        private static List<string> ListFile(Package p, string prefix, string[] notPostfix)
        {
            return p.FileList.Keys.Where(k => k.StartsWith(prefix) && !notPostfix.Any(pp => k.EndsWith(pp))).ToList();
        }

        private static List<string> ListSubDirectories(Package p, string prefix)
        {
            HashSet<string> result = new HashSet<string>();
            foreach (var file in p.FileList.Keys)
            {
                if (!file.StartsWith(prefix))
                {
                    continue;
                }
                int nextSlash = file.IndexOf('/', prefix.Length);
                if (nextSlash > 0)
                {
                    result.Add(file.Substring(0, nextSlash));
                }
            }
            return result.ToList();
        }

        private static void ExtractFileAtDirectory(Package p, string prefix, string dest)
        {
            foreach (var file in p.FileList.Where(f => f.Key.StartsWith(prefix) && f.Key.IndexOf('/', prefix.Length) == -1))
            {
                SaveFileInPackage(p, file.Key, Path.Combine(dest, file.Key));
            }
        }

        private static void ExtractSubDirectory(Package p, string prefix, string dest)
        {
            foreach (var file in p.FileList.Where(f => f.Key.StartsWith(prefix)))
            {
                SaveFileInPackage(p, file.Key, Path.Combine(dest, file.Key));
            }
        }

        private static IEnumerable<string> GetFileListInPackage(Package pkg, string prefix)
        {
            return pkg.FileList.Keys.Where(str => str.StartsWith(prefix));
        }

        private static void SaveFileInPackage(Package pkg, string filePath, string destFilePath)
        {
            var data = pkg.FileList[filePath];
            var len = data.Length;
        ...
        public static PackageFileInfo GetFileInfo(IFileContentProvider file)
        {
            if (file is StreamDecryptProvider p)
            {
            ...
            return default(PackageFileInfo);
        }

        public static IFileContentProvider RetriveFile(Stream file, PackageFileInfo info)
        {
            return new StreamDecryptProvider(file, info.Offset, info.Length);
        }
    }
}
